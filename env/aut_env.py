...
from resource.resource import (init_resource, append_resource, get_resource,
                               judge_resource, compute_resource_sensitivities)
...
class AndroidAppEnv(gym.Env):
    def __init__(self, app_name, app_package_name, main_activity, al, activities, version, device_name, port, start_time, resource):
        self.al = al  # 使用的算法
        ...
        if os.path.exists('result') and os.path.exists(f'result/{app_name}') and os.path.exists(
            ...
            try:
                with open(f"result/{app_name}/views.json", "r") as fp:
                    self.widget_list = json.load(fp)  # 元素id列表，id具有唯一性, value记录index
            except:
                self.widget_list = defaultdict(int)
        else:
            self.widget_list = defaultdict(int)
    ...
    def check_activity(self):
        ...
        if self.al == "resources":
            ...
            resource_sensitivity = compute_resource_sensitivities(self.resource, temp_resource, self.weight)
            ...
            if self.resource_type == 'all':
                reward = compute_resource_sensitivities(self.resource, later_resource, {key: 1 for key in self.resource.keys()})
            return False, reward
        else:
            if self.current_activity and (self.current_activity not in self.list_activities or self.is_doubted_state()):
                self.list_activities[self.current_activity].append([self.get_tuple_observation(), time.time() - self.start_time, 0, 0])
