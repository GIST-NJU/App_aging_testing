...
def get_resource(package_name, device_name=None):
...
def compute_resource_sensitivities(resource1, resource2, resource_type_weight):
    reward = 0
...
def init_resource(package_name, device_name=None):
    gc(package_name, device_name)
    resources = get_resource(package_name, device_name)
    for k, v in resources.items():
        resources[k] = [v]
    return resources


def append_resource(package_name, pre_resource, device_name=None):
    gc(package_name, device_name)
    resources = get_resource(package_name, device_name)
    for k, v in resources.items():
